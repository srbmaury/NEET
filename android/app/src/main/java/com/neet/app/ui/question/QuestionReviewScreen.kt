...
fun QuestionReviewScreen(
    ...
    val viewModel: QuestionReviewViewModel = viewModel(
        factory = QuestionReviewViewModelFactory(historyRepository, answerId),
    )
        ...
        Box(modifier = Modifier.fillMaxSize().padding(innerPadding).padding(16.dp)) {
            ...
            if (current == null) {
                ...
                val readyState = QuestionUiState.Ready(
                    question = question,
                    selectedOptionKey = current.selectedOptionKey,
                    isAnswered = true,
                )
